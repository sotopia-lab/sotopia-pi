...
def create_infer_tab(engine: "Engine") -> Dict[str, "Component"]:
    ...
    elem_dict.update(
        dict(load_btn=load_btn, unload_btn=unload_btn, info_box=info_box)
    )

    chat_box, chatbot, history, chat_elems = create_chat_box(
    ...
    ).then(lambda: ([], []), outputs=[chatbot, history]).then(
        lambda: gr.update(visible=engine.chatter.loaded), outputs=[chat_box]
    )
