...
class Engine:
    def __init__(self, pure_chat: Optional[bool] = False) -> None:
        ...
        self.chatter: "WebChatModel" = WebChatModel(
            manager=self.manager, lazy_init=(not pure_chat)
        )

    def _form_dict(self, resume_dict: Dict[str, Dict[str, Any]]):
        return {
            self.manager.get_elem_by_name(k): gr.update(**v)
