...
class DataArguments:
    ...
    streaming: Optional[bool] = field(
        default=False, metadata={"help": "Enable dataset streaming."}
    )
    ...
    sft_packing: Optional[bool] = field(
        default=False,
        metadata={
            "help": "Packing the questions and answers in the supervised fine-tuning stage."
        },
    )
    ...
    def init_for_training(self, seed: int):  # support mixing multiple datasets
        self.seed = seed
        dataset_names = (
            [ds.strip() for ds in self.dataset.split(",")]
