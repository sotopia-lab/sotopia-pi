...
class FinetuningArguments:
    ...
    neft_alpha: Optional[float] = field(
        default=0,
        metadata={
            "help": "The alpha parameter to control the noise magnitude in NEFTune."
        },
    )
    wandb_token: Optional[str] = field(
        default=None, metadata={"help": "The login api token for wandb."}
    )
    ...
    wandb_tags: Optional[List[str]] = field(
        default=None, metadata={"help": "The tag for the current wandb run."}
    )
    ...
    improve_step: Optional[int] = field(
        default=0, metadata={"help": "The n-th improve step in ReST."}
    )
    ...
    def __post_init__(self):
        ...
        if isinstance(self.additional_target, str):
            self.additional_target = [
                target.strip() for target in self.additional_target.split(",")
            ]
