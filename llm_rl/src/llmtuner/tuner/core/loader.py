...
from llmtuner.extras.logging import get_logger, reset_logging
...
require_version(
    ...
    'To fix: pip install "transformers>=4.31.0,<4.35.0"',
)
...
def load_model_and_tokenizer(
    ...
    if getattr(config, "model_type", None) == "qwen":
        for dtype_name, dtype in [
            ...
            setattr(
                config,
                dtype_name,
                getattr(config, "torch_dtype", None) == dtype,
            )
    ...
    if isinstance(model, PreTrainedModel) and "GenerationMixin" not in str(
    ...
    if stage == "rm" or stage == "ppo":
        ...
        if (
            ...
            logger.warning(
                "Only the last checkpoint containing valuehead will be loaded."
            )
