...
def get_train_args(
    ...
    if (
        ...
        raise ValueError(
            "RM and PPO stages can only be performed with the LoRA method."
        )
    ...
    if finetuning_args.stage in ["rm", "dpo"]:
        for dataset_attr in data_args.dataset_list:
            if not dataset_attr.ranking:
                raise ValueError(
                    "Please use ranked datasets for reward modeling or DPO training."
                )
    ...
    if model_args.checkpoint_dir is not None:
        ...
        if model_args.quantization_bit is not None:
            if len(model_args.checkpoint_dir) != 1:
                raise ValueError(
                    "Quantized model only accepts a single checkpoint. Merge them first."
                )

            if not finetuning_args.resume_lora_training:
                raise ValueError(
                    "Quantized model cannot create new LoRA weight. Merge them first."
                )
