        ...
        if isinstance(input_embed, torch.nn.Embedding):
            ...
            def noisy_forward(
                ...
                if self.training:
                    ...
                    embeddings += torch.zeros_like(embeddings).uniform_(
                        -mag_norm, mag_norm
                    )
                return embeddings
            ...
            logger.info(
                "Using noisy embedding with alpha={:.2f}".format(
                    finetuning_args.neft_alpha
                )
            )
        else:
            logger.warning(
    ...
    if finetuning_args.finetuning_type != "full" and hasattr(model, output_layer_name):
        ...
        if isinstance(output_layer, torch.nn.Linear):
            ...
            def forward_in_fp32(self, x: torch.Tensor) -> torch.Tensor:
                return output_layer.__class__.forward(
                    self, x.to(output_layer.weight.dtype)
                ).to(torch.float32)
