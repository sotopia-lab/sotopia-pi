...
from llmtuner.tuner.ppo.utils import (
    dump_layernorm,
    replace_model,
    restore_layernorm,
)
...
class CustomPPOTrainer(PPOTrainer, Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert isinstance(self.log_callback, LogCallback) and isinstance(
            self.save_callback, SavePeftModelCallback
        )
    ...
    def get_inputs(
        ...
        query, response = (
            ...
            response[:, batch["input_ids"].size(-1) :].detach().cpu(),
        )
        queries, responses = [], []
    ...
    def get_rewards(
        ...
        with torch.cuda.amp.autocast(
            dtype=self.model_args.compute_dtype
        ):  # support bf16
