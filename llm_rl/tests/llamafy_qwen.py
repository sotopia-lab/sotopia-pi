...
def save_weight(input_dir: str, output_dir: str, shard_size: str) -> str:
    ...
    for key, value in qwen_state_dict.items():
        ...
        if "wte" in key:
            ...
            if "attn.c_attn" in key:
                ...
                llama2_state_dict[key.replace("attn.c_attn", "self_attn.q_proj")] = (
                    value[:proj_size, ...]
                )
                llama2_state_dict[key.replace("attn.c_attn", "self_attn.k_proj")] = (
                    value[proj_size : 2 * proj_size, ...]
                )
                llama2_state_dict[key.replace("attn.c_attn", "self_attn.v_proj")] = (
                    value[2 * proj_size :, ...]
                )
            elif "attn.c_proj" in key:
                llama2_state_dict[key.replace("attn.c_proj", "self_attn.o_proj")] = (
                    value
                )
                llama2_state_dict[
                ...
                llama2_state_dict[key.replace("ln_2", "post_attention_layernorm")] = (
                    value
                )
            elif "mlp.w1" in key:
    ...
    if index is None:
        ...
        with open(
            os.path.join(output_dir, WEIGHTS_INDEX_NAME), "w", encoding="utf-8"
        ) as f:
...
def llamafy_qwen(input_dir: str, output_dir: str, shard_size: str):
    try:
