...
def get_agent_info(agent1_pk, agent2_pk, env_pk, agent_dict, env_dict):
    ...
    agent1_goal = (
        env_dict[env_pk]["agent_goals"][0]
        .replace("<extra_info>", "")
        .replace("</extra_info>", "")
    )
    agent2_goal = (
        env_dict[env_pk]["agent_goals"][1]
        .replace("<extra_info>", "")
        .replace("</extra_info>", "")
    )

    agent1_goal = (
        agent1_goal.replace("Agent1", agent1_name)
        .replace("Agent2", agent2_name)
        .replace("agent2", agent2_name)
        .replace("agent1", agent1_name)
    )
    agent2_goal = (
        agent2_goal.replace("Agent2", agent2_name)
        .replace("Agent1", agent1_name)
        .replace("agent1", agent1_name)
        .replace("agent2", agent2_name)
    )
...
def fill_template(agent1_info, agent2_info, scenario):
    ...
    scenario = scenario.replace("agent2", agent2_info["agent_name"]).replace(
        "agent1", agent1_info["agent_name"]
    )
    scenario = scenario.replace("Agent2", agent2_info["agent_name"]).replace(
        "Agent1", agent1_info["agent_name"]
    )
...
for key, data in all_json_data.items():
    ...
    if "EnvAgentComboStorage" in key:
        ...
        agent1_info, agent2_info = get_agent_info(
            agent_ids[0], agent_ids[1], env_id, agent_dict, env_dict
        )
        full_prompt = fill_template(
            agent1_info, agent2_info, env_dict[env_id]["scenario"]
        )
        full_prompts.append({"text": full_prompt})
        full_prompt = fill_template(
            agent2_info, agent1_info, env_dict[env_id]["scenario"]
        )
        full_prompts.append({"text": full_prompt})
