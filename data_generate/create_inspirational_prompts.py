...
def collect_social_iqa():
    ...
    with jsonlines.open(os.getcwd()+'/data_generate/env_files/social_iqa_train.jsonl', 'r') as f:
...
def delete_sotopia_data(inspirational_prompt_df):
    sotopia_prompts = []
    sotopia_prompt_df = pd.read_csv(os.getcwd()+"/data_generate/env_files/inspirational_prompt_for_sotopia.csv")
    sotopia_prompts = sotopia_prompt_df.prompt.tolist()
    inspirational_prompt_df = inspirational_prompt_df[~inspirational_prompt_df['prompt'].isin(sotopia_prompts)]
