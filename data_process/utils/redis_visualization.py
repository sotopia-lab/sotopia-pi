...
def plot_agent_reward_distribution(env_rewards, scorename):
    ...
    axs[0].legend(labels=["Mean {}, Median {}, Var {}".format
    ...
    axs[1].legend(labels=["Mean {}, Median {}, Var {}".format
...
def plot_env_reward_distribution(env_rewards, split_agent=True):
    ...
    if split_agent:
         num_env *=2
    fig, axs = plt.subplots((num_env//4)+1, 4, figsize=(25, 5*((num_env//4)+1)))
...
def make_pretty(styler, varmax):
    ...
    styler.background_gradient(subset=[('agent1','var'), ('agent2','var')], vmin=0, vmax=varmax, cmap="YlGnBu")
    styler.background_gradient(subset=[('agent1','mean'), ('agent2','mean')], vmin=0, vmax=10, cmap="PuBu")
    return styler