...
from redis_visualization import (
    make_pretty,
    plot_agent_reward_distribution,
    plot_env_reward_distribution,
)
...
OTHER_SCENARIO_PK = [
    "01H4EFKY8CXBETGNDNDD08X2MS",
    "01H4EFKY8H8A4P12S4YYE2DNCC",
    "01H4EFKY8VCAJJJM8WACW3KYWE",
    "01H4EFKY91BHEG5GD3VRGGY9YE",
    "01H6S9W1B2QRVBT0JTZTX8DVEM",
    "01H6S9W1B9KPXARNHRFTXBAQ6A",
    "01H6S9W1BFRRT091TCP4E4E66J",
    "01H6S9W1BMGR7MFRPH0V55J2TD",
    "01H6S9W1BSQK5WT5Y9RAKSH45J",
    "01H6S9W1BZVYXYSDG4KR2Z3F4X",
]
...
env_episodes = get_episode_by_env(
    TAG_LIST, USE_ONLY_SOTOPIA, USE_ONLY_GEN, OTHER_SCENARIO_PK
)
env_rewards, env_pks = goal_reward_by_env_agent(env_episodes, FILTER_SCORE)
env_mean_var = get_env_mean_var(env_rewards)

"""Threshold Checking"""
approx_threshold = get_threshold_by_keep_rate(
    env_rewards, KEEP_RATE, IF_BALANCE
)
...
# display strats
filter_stats_df = pd.concat(
    {k: pd.DataFrame(v) for k, v in filter_env_mean_var.items()}
).unstack(1)
...
print(
    "Total {} + {} Conversation Across {} Environments.".format(
        filter_agent1_num,
        filter_agent2_num,
        len(filter_stats_df),
    )
...
print(
    "Agent 1 reduced by {} ({}% of original). Agent 2 reduced by {} ({}% of original).".format(
        agent1_num - filter_agent1_num,
        100 * round((filter_agent1_num) / (agent1_num), 2),
        agent2_num - filter_agent2_num,
        100 * round((filter_agent2_num) / (agent2_num), 2),
    )
)
if filter_agent1_num != filter_agent2_num:
    print(
        "If we want to keep conversations for agent 1 and agent 2 balance, go to the above cell and change BALANCE = True"
    )
    print(
        "Resulting number of conversations would be no more than {} for each agent.".format(
            min(filter_agent1_num, filter_agent2_num)
        )
    )
filter_stats_df.style.pipe(
    make_pretty,
    max(filter_stats_df["agent1"]["var"] + filter_stats_df["agent2"]["var"]),
)
