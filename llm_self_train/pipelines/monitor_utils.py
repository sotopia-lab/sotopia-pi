...
def overwrite_deploy_bash():
    ...
    """
    with open(
        os.path.join(config["script_dir"], "pipelines", "submit_deploy.sh"),
        "r",
    ) as f:
        lines = f.readlines()
    with open(
        ...
        for line in lines:
            if (
                line.strip().startswith("python3")
                and "monitor_deploy_and_run_eval.py" in line
            ):
                line = f"python3 {os.path.join(config['script_dir'], 'pipelines', 'monitor_deploy_and_run_eval.py')} &\n"
            if (
                line.strip().startswith("python3")
                and "monitor_eval_and_stop_deploy.py" in line
            ):
                line = re.sub(
                    r"--ckpt-name \S+",
                    f'--ckpt-name {deploy_config["ckpt_name"]}',
                    line,
                )
            if "--model-path" in line:
                line = re.sub(
                    r"--model-path \S+",
                    f'--model-path {os.path.join(config["checkpoint_dir"], config["experiment_name"], deploy_config["ckpt_name"])}',
                    line,
                )
            if "--port" and "fastchat.serve.controller" in line:
                line = re.sub(
                    r"--port \d+",
                    f'--port {deploy_config["controller_port"]}',
                    line,
                )
            if "--port" and "fastchat.serve.model_worker" in line:
                line = re.sub(
                    r"--port \d+",
                    f'--port {deploy_config["worker_port"]}',
                    line,
                )
            if "--port" and "fastchat.serve.openai_api_server" in line:
                line = re.sub(
                    r"--port \d+",
                    f'--port {deploy_config["deploy_port"]}',
                    line,
                )
            if "--worker-address" in line:
                line = re.sub(
                    r"--worker-address \S+",
                    f'--worker-address http://localhost:{deploy_config["worker_port"]}',
                    line,
                )
            if "--controller-address" in line:
                line = re.sub(
                    r"--controller-address \S+",
                    f'--controller-address http://localhost:{deploy_config["controller_port"]}',
                    line,
                )
            f.write(line)
...
def overwrite_eval_gin():
    ...
    if not os.path.isfile(
        os.path.join(log_dir, f"generate_{deploy_config['ckpt_name']}.gin")
    ):
        source_gin_file = "resources/generate.gin"
        shutil.copy(
            source_gin_file,
            log_dir + "/" + f"generate_{deploy_config['ckpt_name']}.gin",
        )
    ...
    with open(
        os.path.join(
            ...
            f"generate_{deploy_config['ckpt_name']}.gin",
        ),
        "r",
    ) as f:
        lines = f.readlines()
    ...
    for line in lines:
        if line.startswith("CUSTOM_OPENAI_API_BASE"):
            modified_lines.append(
                f"""CUSTOM_OPENAI_API_BASE="http://0.0.0.0:{deploy_config['deploy_port']}/v1"\n"""
            )
        elif line.startswith("CUSTOM_MODEL_NAME"):
            modified_lines.append(
                f"""CUSTOM_MODEL_NAME="{deploy_config['ckpt_name']}"\n"""
            )
        else:
...
def overwrite_eval_bash():
    ...
    if not os.path.isfile(
        ...
        shutil.copy(
            source_gin_file,
            log_dir + "/" + f"submit_eval_{deploy_config['ckpt_name']}.sh",
        )
...
def check_log_and_submit_deploy():
    while True:
        ...
        if existing_names:
            ...
            # Submit deploy job
            with open(
                os.path.join(deploy_config["tmp_dir"], "deploy_job_id.txt"),
                "w",
            ) as f:
            ...
            while True:
                if os.path.exists(
                    ...
                    print(
                        f"Error in deploying. Cancelled {get_deploy_job_id()} and try again..."
                    )
                    os.remove(
                        os.path.join(
                            deploy_config["log_dir"],
