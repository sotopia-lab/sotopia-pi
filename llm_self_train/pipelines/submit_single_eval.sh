...
python -m examples.experiment_eval \
    ...
    '--gin.BATCH_SIZE=2' \
    '--gin.TAG="base-sft-round-2_checkpoint_improve-0_epoch-16_gpt-3.5-turbo_test"' \
    '--gin.PUSH_TO_DB=True' \
    '--gin.TAG_TO_CHECK_EXISTING_EPISODES="base-sft-round-2_checkpoint_improve-0_epoch-16_gpt-3.5-turbo_test"'
...
if [ $? -eq 0 ]; then
    echo "Success" > /home/ruiyiwan/sotopia-llm/llm_self_train/logs/base-sft-round-2/eval_status_checkpoint_improve-0_epoch-16.txt
else
    echo "Failed" > /home/ruiyiwan/sotopia-llm/llm_self_train/logs/base-sft-round-2/eval_status_checkpoint_improve-0_epoch-16.txt
fi
