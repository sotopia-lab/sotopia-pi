...
def upload_dir_to_gcp(
    source_dir, destination_blob_prefix, bucket_name=config["bucket_name"]
):
    """
...
def download_dir_from_gcp(
    source_blob_prefix, destination_dir, bucket_name=config["bucket_name"]
):
    """
...
def download_files_from_gcp(
    files_to_download, local_download_dir, bucket_name=config["bucket_name"]
):
    storage_client = Client()
    bucket = storage_client.bucket(bucket_name)
...
def list_blobs_with_prefix(
    directory, prefix, delimiter=None, bucket_name=config["bucket_name"]
):
    """
